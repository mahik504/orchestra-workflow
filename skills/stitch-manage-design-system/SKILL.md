...
If you need to extract a design system from existing screens, use `stitch-extract-design-md` or `taste-design`.
...
1. Map vague terms to precise values using `stitch-generate-design` mappings.
2. Select concrete hex codes, font families, and roundness values.
3. Generate `DESIGN.md` with `taste-design`.
4. Proceed to the "Create or Update Design System in Stitch" step below.

## 📝 Output Structure

Follow `taste-design` for `DESIGN.md` structure.
