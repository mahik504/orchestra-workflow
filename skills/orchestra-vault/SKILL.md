...
description: Maintains the Obsidian vault at <ORCHESTRA_HOME>. Per-project folders. File only lasting notes. Delete junk the same turn. Learn taste and thinking into Preferences.md.
---
...
Polish H1s and YAML `title:` so the vault looks professional when shown. **Do not rename** `memory`, `projects/`, `templates/`, `kit/`, or `<ORCHESTRA_HOME>`.
...
- **Private** `<your-private-vault>`: full backup, including career and unpublished ideas. 12h sync. Do not gitignore career.md here. Do not make this repo public (history leak).
